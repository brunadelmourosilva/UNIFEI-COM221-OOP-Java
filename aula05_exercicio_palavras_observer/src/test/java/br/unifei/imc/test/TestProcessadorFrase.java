...
import br.unifei.imc.processador.observavel.ProcessadorFrase;
...
public class TestProcessadorFrase {
    ...
    public void testeRemoveObservador() {
        ...
        processadorFrase.registrar(new ContaPalavrasPares());
        processadorFrase.processar(FRASE2);
        Assert.assertEquals(0, processadorFrase.getObservadores().get(0).getNumPalavras()); //palavras maiusculas
        Assert.assertEquals(5, processadorFrase.getObservadores().get(1).getNumPalavras()); //palavras imapares
        Assert.assertEquals(6, processadorFrase.getObservadores().get(2).getNumPalavras()); //conta todas as palavras
        Assert.assertEquals(1, processadorFrase.getObservadores().get(3).getNumPalavras()); //conta palavras pares

        processadorFrase.cancelar(obs); //cancelar/retirar da lista de observadores a palavras pares
        Assert.assertEquals(0, processadorFrase.getObservadores().get(0).getNumPalavras());
        Assert.assertEquals(5, processadorFrase.getObservadores().get(1).getNumPalavras());
